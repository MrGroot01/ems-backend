...
def send_payslip_email(email, user_name, month, year, net_salary):
    ...
    try:
        send_mail(
            ...
            message        = f'Your payslip for {month}/{year} has been generated. Net Salary: Rs.{net_salary}',
            from_email     = settings.DEFAULT_FROM_EMAIL,
            recipient_list = [email],
            fail_silently  = False,
            html_message   = f'''
...
def send_leave_status_email(email, user_name, status, leave_type, dates):
    ...
    try:
        ...
        send_mail(
            ...
            message        = f'Your {leave_type} leave has been {status}. Dates: {dates}',
            from_email     = settings.DEFAULT_FROM_EMAIL,
            recipient_list = [email],
            fail_silently  = False,
            html_message   = f'''
        ...
        logger.info(f'Leave status email sent to {email}')
        return True
...
def send_task_reminder_email(email, user_name, task_title, due_date):
    ...
    try:
        send_mail(
            ...
            message        = f'Task {task_title} is due on {due_date}. Please update your progress.',
            from_email     = settings.DEFAULT_FROM_EMAIL,
            recipient_list = [email],
            fail_silently  = False,
            html_message   = f'''
            <div style="font-family:Arial,sans-serif;max-width:500px;
                ...
                <p>Please login to EMS Pro to update your progress.</p>
                <hr/>
                <small style="color:#94a3b8;">EMS Pro</small>
            </div>
            '''
        )
        logger.info(f'Task reminder sent to {email}')
        return True
